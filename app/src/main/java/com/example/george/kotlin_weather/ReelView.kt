package com.example.george.kotlin_weather
...
class ReelView(context: Context, attrs: AttributeSet?) : View(context, attrs) {
    ...
    private fun initAnimator() {
        ...
        disAnimator.addUpdateListener {
            ...
            Log.d(TAG, "faction:" + it.animatedFraction + " dis:" + dis + " width:" + width + " reelWidth:" + reelWidth)
            postInvalidate()
        }
    }

    override fun onDetachedFromWindow() {// 防止内存泄漏
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        canvas?.let {
            ...
            val count = it.saveLayer(null, null, Canvas.ALL_SAVE_FLAG)// 记录当前状态
            paint.xfermode = PorterDuffXfermode(PorterDuff.Mode.DST_ATOP)
            drawText(it)
            drawPaper(it)
            paint.xfermode = null
            drawPaerLines(it)
            it.restoreToCount(count)// 还原到记录的状态
            drawReels(it)
        }
